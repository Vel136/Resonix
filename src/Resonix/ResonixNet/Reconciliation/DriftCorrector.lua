...
function DriftCorrector.UpdateOffset(self: any, ServerTimestamp: number, DeltaTime: number)
	...
	-- Exponential blend. Alpha = clamp(deltaTime * rate, 0, 0.2).
	-- Capped at 0.2 (not 1.0) so that a long frame or pause cannot snap
	-- the offset to the raw sample in one step — worst case is a 20% blend.
	local Alpha  = math_min(DeltaTime * self._ClockSyncRate, 0.2)
	local OldOff = self._ClockOffset
	local Jump   = math_abs(RawOffset - OldOff)

	-- Dampen outlier samples. A jump > 100ms is almost certainly network
	-- jitter rather than true clock drift — blend it in very slowly so
	-- a single bad ping measurement cannot destabilise the offset estimate.
	if Jump > 0.1 then
		Alpha = math_min(Alpha, 0.02)
		Logger:Debug(string_format(
			"DriftCorrector: outlier sample (jump %.3fs) — dampening alpha to %.2f",
			Jump, Alpha
		))
	elseif Jump > 0.05 then
		Logger:Debug(string_format(
