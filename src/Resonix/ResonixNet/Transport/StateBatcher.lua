...
--[[
	...
	Called ONLY on PlayerAdded because sound emissions are static — their
	position doesn't change after emission. The client can reconstruct the
	full emission state from a single snapshot: position, preset, and the
	server-clock timestamps for EmittedAt/ExpiresAt.

	Rather than per-frame replication, we pay a one-time joining cost per
	player to ensure they hear long-duration ambient sounds (vending machines,
	power nodes, helicopter approach) that were already active when they arrived.
