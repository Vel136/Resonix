...
local MAX_BUFFER     = Constants.OUTBOUND_BUFFER_MAX
...
-- Returns true if writing channel(1) + length(2) + payload would exceed the ceiling.
local function WouldOverflow(Cursor: any, PayloadLen: number): boolean
	return Cursor.Offset + 3 + PayloadLen > MAX_BUFFER
end
...
function OutboundBatcher.WriteEmitForAll(
	...
	local PayloadLen = buffer_len(EncodedEmit)
	for _, Player in AllPlayers do
		...
		if WouldOverflow(Cursor, PayloadLen) then
			Logger:Warn(string_format("OutboundBatcher: emit dropped for '%s' — buffer ceiling reached", Player.Name))
			continue
		end
...
function OutboundBatcher.WriteEmitForPlayer(
	...
	if WouldOverflow(Cursor, buffer_len(EncodedEmit)) then
		Logger:Warn(string_format("OutboundBatcher: emit dropped for '%s' — buffer ceiling reached", Player.Name))
		return
	end
	WriteChannelByte(Cursor, CHANNEL_EMIT)
...
function OutboundBatcher.WriteCancelForAll(
	...
	local PayloadLen = buffer_len(EncodedCancel)
	for _, Player in AllPlayers do
		local Cursor = self._Cursors[Player]
		if not Cursor then
			Cursor = NewCursor()
		...
		if WouldOverflow(Cursor, PayloadLen) then
			Logger:Warn(string_format("OutboundBatcher: cancel dropped for '%s' — buffer ceiling reached", Player.Name))
			continue
		end
...
function OutboundBatcher.WriteStateForPlayer(
	...
	if WouldOverflow(Cursor, buffer_len(EncodedState)) then
		Logger:Warn(string_format("OutboundBatcher: state dropped for '%s' — buffer ceiling reached", Player.Name))
		return
	end
