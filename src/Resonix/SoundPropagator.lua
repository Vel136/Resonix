...
--[[
	...
	PERFORMANCE TIERS:
		...
		SIMPLE — single raycast, binary hit/miss with conservative fixed
		         attenuation. Used for 50–100% of max radius.
		         Less accurate, cheap enough to run for all mid-range listeners.
...
SoundPropagator.Accuracy = table.freeze({
	...
	Simple = "simple",  -- single raycast, fixed attenuation on hit
...
-- Fixed attenuation for the Simple accuracy tier.
-- A single wall hit in Simple mode reduces transmission by this fraction.
-- Tuned to approximate the average result of a Full calculation through
-- a typical interior wall (0.4-stud wood/plastic).
local SIMPLE_WALL_ATTENUATION = 0.35
...
function SoundPropagator.ComputeOcclusion(
	...
	-- Simple tier: single raycast, binary hit/miss
	if acc == SoundPropagator.Accuracy.Simple then
		local result = workspace:Raycast(sourcePos, direction, rayParams)
		if not result then
			return 1.0  -- clear line
		end
		-- Hit something — apply fixed conservative attenuation
		return 1.0 - SIMPLE_WALL_ATTENUATION
	end

	-- Full tier: multi-segment ray march with per-material absorption
	local currentOrigin      = sourcePos
	local distanceTraveled   = 0
	local totalAbsorption    = 0
	local segmentCount       = 0
	local maxThicknessProbe  = math.min(totalDistance, 20) -- max 20 studs per thickness measurement
	...
	while distanceTraveled < totalDistance and segmentCount < MAX_MARCH_SEGMENTS do
		...
		-- Add a small nudge (0.05 studs) to avoid re-hitting the same surface
	...
	-- This is the physically correct model for acoustic absorption
	-- through layered materials.
	return math.exp(-totalAbsorption)
end
