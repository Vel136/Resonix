...
--[[
	...
	QueryPosition at discrete intervals (every 0.1 seconds), but sound events
	happen at arbitrary moments in continuous time. An instantaneous event like
	a gunshot that fires at t=0.05 must still be detectable when a query
	...
	ensuring it's visible to at least six consecutive query polls.
]]
...
-- This window ensures that fast-polling consumers (e.g. querying at 0.1s)
...
-- 128 covers: footsteps, weapon fire, impacts, and environment
-- sounds across a full server session with headroom.
...
--[[
	...
	emission would persist in the buffer for its full duration even though
...
function SoundBuffer.IterateNear(
	self      : SoundBuffer,
	position  : Vector3,
	maxRadius : number
): () -> SoundEmission?
