...
-- Bounds worst-case raycast count per call.
--
-- Note on diffraction: each diffraction-enabled emission that is blocked can
-- add up to DIFFRACTION_RAY_COUNT × 2 extra raycasts. This is already factored
-- into the budget — a query with 16 diffraction-enabled emissions in the Full
-- tier will cost more raycasts than one without. If your scene has many
-- simultaneous diffraction emissions, consider lowering MAX_OCCLUSION_PER_QUERY
-- or reducing DIFFRACTION_RAY_COUNT in SoundPropagator.
local MAX_OCCLUSION_PER_QUERY  = 16

-- ─── Built-in Presets ────────────────────────────────────────────────────────

local BUILT_IN_PRESETS: { [string]: EmissionPreset } = {
	-- Weapons — diffraction enabled: these are the sounds AI most needs
	-- to detect around corners. A gunshot in the next corridor should still
	-- register as a weak stimulus.
	Gunshot_Pistol = {
		Intensity   = 65,
		Radius      = 100,
		Duration    = 0.05,
	...
	-- Movement — no diffraction: footsteps are short-range and frequent.
	-- The extra raycast cost per step is not justified for sounds that
	-- NPC detection doesn't need to hear around corners.
	Footstep_Walk = {
		Intensity  = 12,
		Radius     = 25,
		Duration   = 0.05,
	...
	-- Impacts — no diffraction: short-range, high frequency of occurrence.
	Impact_Bullet_Hard = {
		Intensity  = 30,
		Radius     = 50,
		Duration   = 0.05,
	...
	-- Environment — no diffraction: these are persistent/ambient.
	-- They do not represent events that AI needs to locate precisely.
	VendingMachine = {
		Intensity  = 42,
		Radius     = 65,
		Duration   = 60.0,
	...
	-- Explosions — diffraction enabled: low-frequency energy bleeds through
	-- walls and around corners. An AI hearing a grenade go off in the next
	-- room is realistic and important for gameplay.
	Explosion_Grenade = {
		Intensity   = 88,
		Radius      = 280,
		Duration    = 0.1,
...
function Resonix.EmitPreset(
	...
	return self:Emit({
		...
		PresetKey   = presetKey,
		Diffraction = preset.Diffraction,
	})
end
...
function Resonix.Emit(
	...
	local emission: SoundEmission = {
		...
		Tag         = config.Tag,
		Diffraction = config.Diffraction or false,
	}
...
--[[
	Cache key format: "{emissionId}|{quantizedPosition}[|d]"

	The "|d" suffix is appended when the emission has Diffraction = true.
	This ensures diffraction and non-diffraction queries for the same
	emission/position pair are cached separately — important because
	the two can produce different transmission values.

	Without the suffix, a non-diffraction query could poison the cache
	for a subsequent diffraction query at the same position, causing
	the diffracted path to never be found.
]]
local CACHE_KEY_DIFFRACTION_SUFFIX = "|d"
...
function Resonix._GetCachedOcclusion(
	...
	if emission.Diffraction then
		cacheKey = cacheKey .. CACHE_KEY_DIFFRACTION_SUFFIX
	end
	...
	local transmission = SoundPropagator.ComputeOcclusion(
		...
		excludes,
		accuracy,
		emission.Diffraction  -- passed through to propagator
	)
